...
export default function SongCard({ song, onPlay = () => {} }) {
  return (
    <button
      onClick={() => onPlay(song)}
      className="group p-4 rounded-lg bg-background-elevated hover:bg-background-highlight transition-colors text-left w-full"
    >
      {/* Album Art */}
      <div className="relative aspect-square mb-4 rounded-md overflow-hidden bg-background-highlight">
        {song.coverImage ? (
          <Image
        ...
        <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all">
          <div className="w-12 h-12 rounded-full bg-accent flex items-center justify-center shadow-lg hover:scale-105 hover:bg-accent-hover transition-all">
